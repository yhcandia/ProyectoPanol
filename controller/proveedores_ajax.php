<?php
	...
	if($action == 'ajax'){
		...
		$count_query   = mysqli_query($con,"SELECT count(*) AS numrows FROM proveedor ");
		if ($row= mysqli_fetch_array($count_query)){$numrows = $row['numrows'];}
		$total_pages = ceil($numrows/$per_page);
		$reload = 'proveedorView.php';
		//consulta principal para recuperar los datos
		$query = mysqli_query($con,"SELECT * FROM proveedor LIMIT $offset,$per_page");
		...
		if ($numrows>0){
			?>
                ...
                <div class="panel panel-default col-md-8 center-block">
                    <div class="panel-body ">Datos Proveedor</div>
